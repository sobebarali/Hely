...
function PrescriptionsListPage() {
		...
		usePrescriptions({
			...
			status:
				statusFilter && statusFilter !== "ALL"
					? (statusFilter as PrescriptionStatus)
					: undefined,
		});
	...
	const table = useReactTable({
		...
		pageCount: prescriptionsData?.pagination.totalPages ?? 0,
	});
	...
	return (
		<div className="flex flex-col gap-4 p-4 md:gap-6 md:p-6">
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					<h1 className="font-bold text-2xl">Prescriptions</h1>
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center">
				<div className="relative flex-1">
					...
					<Input
						placeholder="Search by prescription ID, patient, or diagnosis..."
						value={search}
						onChange={(e) => {
			...
			{prescriptionsData && (
				<div className="flex items-center justify-between px-2">
					<div className="text-muted-foreground text-sm">
						Showing {prescriptionsData.data.length} of{" "}
						{prescriptionsData.pagination.total} prescriptions
					</div>
					<div className="flex items-center gap-2">
						...
						<span className="text-sm">
							Page {page} of {prescriptionsData.pagination.totalPages || 1}
						</span>
						<Button
							variant="outline"
							size="icon"
							onClick={() =>
								setPage((p) =>
									Math.min(prescriptionsData.pagination.totalPages, p + 1),
								)
							}
							disabled={page === prescriptionsData.pagination.totalPages}
						>
						...
						<Button
							variant="outline"
							size="icon"
							onClick={() => setPage(prescriptionsData.pagination.totalPages)}
							disabled={page === prescriptionsData.pagination.totalPages}
						>
