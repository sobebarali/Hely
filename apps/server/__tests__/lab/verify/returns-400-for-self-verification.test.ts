...
describe("POST /api/lab/orders/:orderId/verify - Returns 400 for self-verification", () => {
	...
	beforeAll(async () => {
		...
		const createResponse = await request(app)
			...
			.send({
				...
				diagnosis: "Self-verification test",
			});

		createdLabOrderId = createResponse.body.id;

		// Force to RESULTS_ENTERED with resultEnteredBy = context.staffId (same person)
	...
	it("returns 400 SELF_VERIFICATION when verifier is the same as result enterer", async () => {
		const payload = {
		...
		expect(response.body.code).toBe("SELF_VERIFICATION");
	});
});
